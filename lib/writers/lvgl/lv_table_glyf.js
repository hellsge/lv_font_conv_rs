...
class LvGlyf extends Glyf {
  ...
  to_lv_bitmaps() {
    ...
    this.lv_data.forEach((d, idx) => {
      if (idx === 0) return;  // Skip reserved id=0
      if (!d || !d.bin) return;  // Skip undefined entries
...
${u.long_dump(d.bin, { hex: true, col: cols })}`;
  ...
  toBinaryFile() {
    ...
    // Concatenate all glyph bitmap data into a single Buffer
    const buffers = [];
    this.lv_data.forEach((d, idx) => {
      if (idx === 0) return; // Skip reserved id=0
      if (d && d.bin && d.bin.length > 0) {  // Only add if data exists and has content
        buffers.push(d.bin);
      }
    });
  ...
  toLVGLWithExternalBitmap(macroName) {
    ...
    const macroNameUpper = macroName.toUpperCase();
 ...
 *Define ${macroNameUpper}_GLYPH_BITMAP_BIN as the memory address where the binary is loaded.*/
#ifndef ${macroNameUpper}_GLYPH_BITMAP_BIN
#define ${macroNameUpper}_GLYPH_BITMAP_BIN 0
#warning "Please define ${macroNameUpper}_GLYPH_BITMAP_BIN to the flash memory address"
#endif

static const uint8_t * const glyph_bitmap = (const uint8_t *)${macroNameUpper}_GLYPH_BITMAP_BIN;
